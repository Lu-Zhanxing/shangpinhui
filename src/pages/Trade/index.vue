  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="detail">
        ...
        <ul
          class="list clearFix"
          v-for="(good, index) in tradeGoodList.detailArrayList"
          :key="good.skuId"
        >
          <li>
            <img width="100px" height="100px" :src="good.imgUrl" alt="" />
          </li>
          <li>
            <p>{{ good.skuName }}</p>
            <h4>7天无理由退货</h4>
    ...
    <div class="money clearFix">
      <ul>
        <li>
          <b
            ><i>{{ tradeGoodList.totalNum }}</i
            >件商品，总商品金额</b
          >
          <span>¥{{ tradeGoodList.totalAmount }}</span>
        </li>
        <li>
          <b>返现：</b>
          <span>0.00</span>
    ...
    <div class="trade">
      <div class="price">
        应付金额:　<span>¥{{ tradeGoodList.totalAmount }}</span>
      </div>
...
export default {
  ...
  mounted() {
    ...
    this.$store.dispatch("getTradeGoodList");
  },
  computed: {
    ...mapGetters(["userAddress", "tradeGoodList"]),
    selectedAddress() {
  ...
  methods: {
    // 改变默认地址
    changeDefaultAddress(current, userAddress) {
    ...
    // 提交订单
    async submitOrder() {
      let { tradeNo, detailArrayList} = this.tradeGoodList;
      let {consignee,phoneNum,fullAddress} = this.selectedAddress
      let data = {
        consignee: consignee,
